...
data = pd.DataFrame(
    {
        ...
        "target": [
            np.nan, -3.5, -3.5, np.nan, -4.0, -3.1, -3.9, np.nan, -3.9,
            -3.6, np.nan, np.nan, np.nan, np.nan, -3.9, np.nan, -3.8,
        ],
        "realised": [
            np.nan, np.nan, np.nan, -4.3, -6.1, -6.0, -6.6, -5.1, -4.0,
            -4.1, -1.0, -3.8, -0.8, -0.0, -3.4, -5.0, np.nan,
        ],
    }
)

PLOT_DATA = data.assign(target=data["target"].abs(), realised=data["realised"].abs())
PLOT_DATA = PLOT_DATA[PLOT_DATA["year"] >= 2011].copy()
...
def make_carbon_intensity_plot():
    ...
    ax.set_title(i18n("Carbon intensity reduction"), loc="left", fontweight="bold", fontsize=12)
